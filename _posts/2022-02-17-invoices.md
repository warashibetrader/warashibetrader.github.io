...
The makePaymentURL function returns the URL for an invoice based on the three input parameters. Use this URL for your payment link/button. For instance, using the same HTML button from Step 2:
...
The following example creates a thank-you alert when the client completes a payment:
	...
	document.getElementById("myInvoiceElement").onclick = function() {
		startPayment(preferred, addresses, items, thankYouConfirmation);
	};
