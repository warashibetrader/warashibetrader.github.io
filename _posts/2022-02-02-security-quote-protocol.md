...
The problem with using web apps for secure applications is that every time you visit the app, its code is newly downloaded and may be different from the trusted version you used last. The fact that the possibly foreign app doesn't disappear with your private information in hand is not an expectation - just a pleasant surprise.

The _security quote protocol_ solution in a nutshell: The app's client code selects a random quote or image on first visit, and does NOT expose the selection to the server. It shows this quote to the user each time the app loads. If the app is updated, the quote is deleted immediately. The user then knows that the app's code has been updated, and proceeds with caution.
